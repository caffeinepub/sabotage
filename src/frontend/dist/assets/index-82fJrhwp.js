...
if (!document.getElementById("cod-fonts")) {
  ...
  link.id = "cod-fonts";
  link.rel = "stylesheet";
  link.href = "https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;600;700&family=Oswald:wght@400;600;700&display=swap";
...
const MAPS = {
  alcatraz: {
    id: "alcatraz",
    label: "ALCATRAZ",
    description: "Maximum security island prison. Multiple wings & role stations.",
    playerSpawn: { x: 0, z: 0 },
    // Outer perimeter + internal room dividers
    walls: [
      // Outer walls
      { x: -28, z: -28, w: 56, h: 2, d: 2 },
      // north
      { x: -28, z: 26, w: 56, h: 2, d: 2 },
      // south
      { x: -28, z: -28, w: 2, h: 2, d: 56 },
      // west
      { x: 26, z: -28, w: 2, h: 2, d: 56 },
      // east
    ...
    tasks: [
      { x: -20, z: 10, name: "FIX GENERATOR" },
      // Engineer Bay
      { x: -22, z: 18, name: "REPAIR PIPES" },
      // Engineer Bay
      { x: 18, z: 10, name: "SCAN PATIENT" },
      // Medbay
      { x: 14, z: 20, name: "MIX ANTIDOTE" },
      // Medbay
      { x: -23, z: -24, name: "GUARD PATROL LOG" },
      // Guard Tower
      { x: 20, z: -24, name: "RESTOCK AMMO" },
      // Armory
      { x: -15, z: -8, name: "UNLOCK CELL A4" },
      // Cell Block A
      { x: 15, z: -8, name: "UNLOCK CELL B7" },
      // Cell Block B
      { x: 0, z: -18, name: "CALIBRATE CAMERAS" },
      // Courtyard
      { x: 0, z: 14, name: "FIX COURTYARD LIGHT" }
      // Courtyard
    ],
    bots: [
      {
  ...
  testing: {
    ...
    description: "Open flat arena. All rooms visible, ideal for testing mechanics.",
    playerSpawn: { x: 0, z: 0 },
    walls: [
      // Outer boundary
    ...
    tasks: [
      { x: -14, z: -16, name: "CALIBRATE TOOLS" },
      // Engineer Room
      { x: -8, z: -18, name: "CHARGE BATTERY" },
      // Engineer Room
      { x: 14, z: -16, name: "HEAL PATIENT" },
      // Doctor Room
      { x: 8, z: -18, name: "ADMINISTER DRUG" },
      // Doctor Room
      { x: 0, z: -2, name: "FIX TERMINAL" },
      // Spawn
      { x: -14, z: 14, name: "SORT SUPPLIES" },
      // Storage
      { x: 14, z: 14, name: "WEAPONS CHECK" },
      // Test Range
      { x: -18, z: 8, name: "UPLOAD DATA" },
      // Storage
      { x: 18, z: 8, name: "TARGET PRACTICE" },
      // Test Range
      { x: 0, z: 18, name: "RESET SYSTEMS" }
      // Bottom area
    ],
    bots: [
      {
...
function checkCollision(x, z, walls) {
  for (const w of walls) {
    if (x + 0.3 > w.x && x - 0.3 < w.x + w.w && z + 0.3 > w.z && z - 0.3 < w.z + w.d)
      return true;
  }
  return false;
}
...
function SabotageGame() {
  ...
  const roleBannerRef = reactExports.useRef(null);
  const locationTextRef = reactExports.useRef(null);
  const tasksTextRef = reactExports.useRef(null);
  const statusTextRef = reactExports.useRef(null);
  const aliveTextRef = reactExports.useRef(null);
  const taskProgressRef = reactExports.useRef(null);
  const killBtnRef = reactExports.useRef(null);
  const reportBtnRef = reactExports.useRef(null);
  const interactionRef = reactExports.useRef(null);
  const hitMarkerRef = reactExports.useRef(null);
  const notificationRef = reactExports.useRef(null);
  const startScreenRef = reactExports.useRef(null);
  const settingsScreenRef = reactExports.useRef(null);
  const mobileControlsRef = reactExports.useRef(null);
  const mobileKillBtnRef = reactExports.useRef(null);
  ...
  const botsRef = reactExports.useRef([]);
  const tasksRef = reactExports.useRef([]);
  const deadBodiesRef = reactExports.useRef([]);
  ...
  const joystickRef = reactExports.useRef({ x: 0, y: 0, active: false });
  const lookTouchRef = reactExports.useRef(null);
  ...
  const meetingActiveRef = reactExports.useRef(false);
  const canReportRef = reactExports.useRef(false);
  const witnessedKillRef = reactExports.useRef(false);
  const lockJustAcquiredRef = reactExports.useRef(false);
  const [meetingActive, setMeetingActive] = reactExports.useState(false);
  const [votingPhase, setVotingPhase] = reactExports.useState(
    null
  );
  const [votes, setVotes] = reactExports.useState({});
  const [myVote, setMyVote] = reactExports.useState(null);
  const [voteResult, setVoteResult] = reactExports.useState(null);
  const [meetingTimer, setMeetingTimer] = reactExports.useState(30);
  const [currentScreen, setCurrentScreen] = reactExports.useState("start");
  const [selectedMap, setSelectedMap] = reactExports.useState("alcatraz");
  const [selectedCharacter, setSelectedCharacter] = reactExports.useState(1);
  const playerColorRef = reactExports.useRef("#cc2200");
  const selectedCharacterRef = reactExports.useRef(1);
  const [perspective, setPerspective] = reactExports.useState(0);
  const perspectiveRef = reactExports.useRef(0);
  const meetingTimerIntervalRef = reactExports.useRef(
    null
  );
  const myVoteRef = reactExports.useRef(null);
  ...
  function closeMeeting() {
    ...
    if (canvas && !isMobileRef.current && gameRef.current.isPlaying && !gameRef.current.gameOver) {
  ...
  reactExports.useEffect(() => {
    ...
    function castRayFull(angle, originX, originZ) {
      let dist = 0;
      const step = 0.05;
      const maxDist = 30;
      const sinA = Math.sin(angle);
      const cosA = Math.cos(angle);
      const px = originX !== void 0 ? originX : playerRef.current.x;
      const pz = originZ !== void 0 ? originZ : playerRef.current.z;
      while (dist < maxDist) {
        const hx = px + sinA * dist;
        const hz = pz - cosA * dist;
        for (const wall of ACTIVE_MAP.walls) {
          if (hx + 0.3 > wall.x && hx - 0.3 < wall.x + wall.w && hz + 0.3 > wall.z && hz - 0.3 < wall.z + wall.d) {
    ...
    function cellNoise(cx, cz) {
      const n = Math.sin(cx * 127.1 + cz * 311.7) * 43758.5453;
      return n - Math.floor(n);
    }
    function wallTexColor(wallX, wallPY, wallHeight, brightness, isDoor) {
      const texH = 64;
      const texW = 64;
      const tx = Math.floor(wallX * texW) % texW;
      const ty = Math.floor(wallPY / wallHeight * texH * 3) % texH;
      if (isDoor) {
        const barCycle = tx % 8;
        if (barCycle < 4) {
      ...
      const base = 88 + Math.floor(noise * 24);
      const r = Math.floor(base * brightness);
      const g = Math.floor((base - 3) * brightness);
      const b = Math.floor((base - 6) * brightness);
      return `rgb(${r},${g},${b})`;
    }
    function floorTexColor(rx, rz, brightness) {
      const cx = Math.floor(rx / 1.5);
      const cz = Math.floor(rz / 1.5);
      const n = cellNoise(cx, cz);
      if (n < 0.45) {
        const v22 = cellNoise(cx * 3 + 1, cz * 3 + 1);
        const r = Math.floor((55 + v22 * 12) * brightness);
        const g = Math.floor((68 + v22 * 20) * brightness);
        const b = Math.floor((40 + v22 * 8) * brightness);
        return `rgb(${r},${g},${b})`;
      }
      if (n < 0.65) {
    ...
    function render() {
      ...
      for (let i = 0; i < numRays; i++) {
        ...
        const wallPixStep = 2;
        for (let py = Math.max(0, Math.floor(wallTop)); py < Math.min(h, Math.ceil(wallBottom)); py += wallPixStep) {
          const wallPY = py - wallTop;
    ...
    function drawPlayerSprite(w, h, pMode) {
      const char = CHARACTERS.find((c) => c.id === selectedCharacterRef.current) || CHARACTERS[0];
      const baseSize = pMode === 2 ? 120 : 80;
      const cx = w / 2;
    ...
    function updatePhysics() {
      ...
      const speed = player.isSprinting || joyMag > 0.5 ? baseSpeed * 2 : baseSpeed;
      let targetVx = 0;
      let targetVz = 0;
      ...
      if (Math.abs(targetVx) < 1e-4 && Math.abs(targetVz) < 1e-4) {
        player.vx *= 0.7;
        player.vz *= 0.7;
      }
      ...
      if (moveMag > 2e-3 && perspectiveRef.current === 2) {
        const targetYaw = Math.atan2(player.vx, -player.vz);
        const yawDiff = (targetYaw - player.yaw + Math.PI * 3) % (Math.PI * 2) - Math.PI;
      ...
      for (const bot of botsRef.current) {
        ...
        if (incompleteTasks.length > 0) {
          const target = incompleteTasks[bot.targetTaskIndex % incompleteTasks.length];
    ...
    function checkInteractions() {
      ...
      for (const t of tasksRef.current) {
        ...
        if (dist < 2 && !t.completed) {
          near = true;
          prompt.textContent = isMobile ? `TAP USE TO ${t.name}` : `PRESS E TO ${t.name}`;
      ...
      if (!near && g.isImpostor && g.killCooldown <= 0) {
        for (const b of botsRef.current) {
          ...
          if (dist < 1.5) {
            near = true;
            prompt.textContent = isMobile ? "TAP KILL TO ELIMINATE" : "PRESS Q TO ELIMINATE";
    ...
    function startGame() {
      ...
      if (isMobile) {
        (_b2 = (_a2 = document.documentElement).requestFullscreen) == null ? void 0 : _b2.call(_a2);
        (_d2 = (_c2 = screen.orientation) == null ? void 0 : _c2.lock) == null ? void 0 : _d2.call(_c2, "landscape");
      } else {
        canvas.requestPointerLock();
      }
    ...
    window.__sabotageStartGame = startGame;
    function onKeyDown(e) {
      ...
      if (e.key.toLowerCase() === "v") {
        perspectiveRef.current = (perspectiveRef.current + 1) % 3;
    ...
    function onMouseMove(e) {
      if (document.pointerLockElement === canvas && !isMobile && !lockJustAcquiredRef.current) {
        playerRef.current.yaw += e.movementX * 2e-3;
        playerRef.current.pitch -= e.movementY * 2e-3;
    ...
    function onClick() {
      if (gameRef.current.isPlaying && document.pointerLockElement !== canvas && !isMobile && !meetingActiveRef.current) {
        canvas.requestPointerLock();
      }
    }
    ...
    killBtn == null ? void 0 : killBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      kill();
    });
    if (isMobile) {
      ...
      let jTouch = null;
      zone == null ? void 0 : zone.addEventListener("touchstart", (e) => {
        e.preventDefault();
        jTouch = e.touches[0];
        updateJoystick(e.touches[0]);
      });
      zone == null ? void 0 : zone.addEventListener("touchmove", (e) => {
        e.preventDefault();
        if (jTouch) {
      ...
      zone == null ? void 0 : zone.addEventListener("touchend", (e) => {
        e.preventDefault();
      ...
      canvas2 == null ? void 0 : canvas2.addEventListener(
        "touchstart",
        (e) => {
        ...
        (e) => {
          ...
          for (const t of Array.from(e.touches)) {
            if (t.identifier === lookTouchRef.current.id) {
              ...
              playerRef.current.yaw += dx * 5e-3;
              playerRef.current.pitch = Math.max(
                -0.5,
                Math.min(0.5, playerRef.current.pitch + dy * 3e-3)
              );
      ...
      sprintBtn == null ? void 0 : sprintBtn.addEventListener("touchstart", (e) => {
        e.preventDefault();
        playerRef.current.isSprinting = true;
      });
      sprintBtn == null ? void 0 : sprintBtn.addEventListener("touchend", (e) => {
        e.preventDefault();
        playerRef.current.isSprinting = false;
      });
      const interactBtn = document.getElementById("sabotage-btn-interact");
      interactBtn == null ? void 0 : interactBtn.addEventListener("touchstart", (e) => {
        e.preventDefault();
        completeTask();
      });
      const mKillBtn = mobileKillBtnRef.current;
      mKillBtn == null ? void 0 : mKillBtn.addEventListener("touchstart", (e) => {
        e.preventDefault();
        kill();
      });
      if (window.DeviceOrientationEvent) {
        window.addEventListener(
          "deviceorientation",
          (e) => {
            if (e.gamma !== null) playerRef.current.yaw += e.gamma * 1e-3;
            if (e.beta !== null)
              playerRef.current.pitch = Math.max(
                -0.5,
                Math.min(0.5, (e.beta - 45) * 5e-3)
              );
          }
        );
      }
  ...
  const [settingsState, setSettingsState] = reactExports.useState({
    moveSpeed: 0.08,
    killCooldown: 10,
    numBots: 3,
    numTasks: 5,
  ...
  function handleStartGame() {
    ...
    setTimeout(() => {
      var _a2;
      (_a2 = window.__sabotageStartGame) == null ? void 0 : _a2.call(window);
    }, 50);
  }
  const getAliveBotNames = () => botsRef.current.filter((b) => b.alive).map((b) => b.name);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-game-vignette" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-game-scanlines" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-compass", children: "N ───── 360° ─────" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-killfeed" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
                  ...
                  onClick: () => {
                    const next = (perspectiveRef.current + 1) % 3;
                  ...
                  children: perspective === 0 ? "1P" : perspective === 1 ? "2P" : "3P"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "sabotage-health", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-health-label", children: "HP" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-health-bar", children: Array.from({ length: 10 }).map((_, i) => (
                  // biome-ignore lint/suspicious/noArrayIndexKey: health segments are positional
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "seg" }, i)
                )) })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "sabotage-ammo", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-ammo-main", children: "30" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "sabotage-ammo-reserve", children: "/ 90" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "sabotage-death-overlay", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "YOU WERE ELIMINATED" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "PRESS ENTER TO CONTINUE" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  style: {
                    ...
                    textTransform: "uppercase",
                    color: "#4aff4a",
                    background: "linear-gradient(90deg, transparent, rgba(10,25,10,0.9), transparent)",
                    border: "none",
                ...
                "div",
                {
                  ...
                  children: [
                    ...
                    "div",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "#0a0", fontSize: 10, marginBottom: 2 }, children: label }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                            ref: id === "location" ? locationTextRef : id === "tasks" ? tasksTextRef : id === "alive" ? aliveTextRef : statusTextRef,
                            style: { fontSize: 16, textShadow: "0 0 10px #0f0" },
                            children: id === "location" ? "MAIN DECK" : id === "tasks" ? "0/5" : id === "alive" ? "4/4" : "ALIVE"
                          }
                        )
                      ]
                    },
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { ref: minimapRef, style: { width: "100%", height: "100%" } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        style: {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  ].map(({ id, ...style2 }) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "div",
                    {
                      style: {
                        position: "absolute",
        ...
        meetingActive && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: "🚨 EMERGENCY MEETING 🚨"
                      }
                    ),
                    votingPhase === "voting" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 14, color: "#0a0", letterSpacing: 3 }, children: "DISCUSS AND VOTE — WHO IS THE SABOTEUR?" })
                  ]
                }
              ),
              votingPhase === "voting" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  style: {
                    width: "100%",
                  ...
                  children: [
                          ...
                          getAliveBotNames().map((botName) => {
                            ...
                            return /* @__PURE__ */ jsxRuntimeExports.jsx(
                              VoteRow,
                              {
                                ...
                                color: ((_a2 = botsRef.current.find((b) => b.name === botName)) == null ? void 0 : _a2.color) || "#fff",
                    ...
                    myVote && /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                        ...
                        children: "VOTE CAST — WAITING FOR RESULTS..."
                      }
                    )
                  ]
                }
              ),
              votingPhase === "result" && voteResult && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  style: {
                    flex: 1,
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: voteResult === "SKIP" ? "NO MAJORITY — SKIP" : voteResult === "PLAYER" ? "YOU WERE EJECTED!" : `${BOT_DISPLAY_NAMES[voteResult] || voteResult} WAS EJECTED!`
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: Object.entries(votes).map(([name, count]) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "div",
                          {
                            style: {
                            ...
                            children: [
                              name === "PLAYER" ? "You" : name === "SKIP" ? "SKIP" : BOT_DISPLAY_NAMES[name] || name,
                              ": ",
                              count,
                              " vote",
                              count !== 1 ? "s" : ""
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: "RESUMING IN 3 SECONDS..."
                      }
                    )
                  ]
                }
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            style: {
              ...
              width: "100%",
              height: "100%",
              background: "linear-gradient(160deg, #0a0b08 0%, #111410 40%, #0d0e0b 100%)",
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "TACTICAL SOCIAL DEDUCTION"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 30, textAlign: "center" }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    style: {
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 16, justifyContent: "center" }, children: Object.values(MAPS).map((m) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    type: "button",
                    ...
                    style: {
                      padding: "14px 22px",
                      minWidth: 160,
                      background: selectedMap === m.id ? "rgba(200,168,75,0.12)" : "rgba(255,255,255,0.03)",
                      color: selectedMap === m.id ? "#c8a84b" : "#7a8a68",
                      border: selectedMap === m.id ? "2px solid #c8a84b" : "2px solid #2a3020",
                      cursor: "pointer",
                      fontFamily: "'Oswald', 'Arial Black', sans-serif",
                      fontSize: 15,
                      letterSpacing: 3,
                      ...
                      textAlign: "left",
                      transition: "all 0.2s",
                      boxShadow: selectedMap === m.id ? "0 0 16px rgba(200,168,75,0.25), inset 0 0 20px rgba(200,168,75,0.05)" : "none"
                    },
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontWeight: "bold", marginBottom: 4 }, children: m.label }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "div",
                        {
                          style: {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 24, width: "100%" }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    style: {
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    ...
                    children: CHARACTERS.map((char) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "button",
                      {
                        type: "button",
                        ...
                        style: {
                          width: 80,
                          height: 100,
                          border: selectedCharacter === char.id ? `2px solid ${char.accent}` : "2px solid rgba(255,255,255,0.15)",
                          borderRadius: 10,
                          background: selectedCharacter === char.id ? `${char.color}33` : "rgba(0,0,0,0.4)",
                          cursor: "pointer",
                          display: "flex",
                          flexDirection: "column",
                          alignItems: "center",
                          justifyContent: "center",
                          gap: 4,
                          boxShadow: selectedCharacter === char.id ? `0 0 12px ${char.accent}88` : "none",
                          transition: "all 0.2s"
                        },
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsxs(
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    onMouseEnter: (e) => {
                      ...
                      e.target.style.boxShadow = "0 0 50px rgba(0,255,0,0.5)";
                    },
                    onMouseLeave: (e) => {
                      e.target.style.background = "transparent";
                      e.target.style.boxShadow = "0 0 30px rgba(0,255,0,0.2)";
                    },
                    children: "DEPLOY"
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    onMouseEnter: (e) => {
                      ...
                      e.target.style.color = "#c8a84b";
                    },
                    onMouseLeave: (e) => {
                      e.target.style.borderColor = "#2a3020";
                      e.target.style.color = "#7a8a68";
                    },
                    children: "SETTINGS"
                  }
                )
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "desktop-hint", children: [
                      ["W", "A", "S", "D"].map((k) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                        {
                          style: {
                            color: "#0f0",
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mobile-hint", style: { display: "none" }, children: "TILT DEVICE TO AIM • JOYSTICK TO MOVE • BUTTONS TO ACT" })
                  ]
                }
              )
            ]
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "CONFIGURE MISSION PARAMETERS"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      SettingRow,
                      {
                        ...
                        onDec: () => updateSetting("numBots", Math.max(1, settingsState.numBots - 1)),
                        onInc: () => updateSetting("numBots", Math.min(6, settingsState.numBots + 1))
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      SettingRow,
                      {
                        ...
                        onDec: () => updateSetting("numTasks", Math.max(3, settingsState.numTasks - 1)),
                        onInc: () => updateSetting(
                          "numTasks",
                      ...
                      "div",
                      {
                        ...
                        children: [
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 8 }, children: [
                            ...
                            {
                              type: "button",
                              onClick: () => updateSetting("btnColors", {
                              ...
                              style: {
                                ...
                                height: 28,
                                borderRadius: "50%",
                                border: settingsState.btnColors[key] === c ? "3px solid #fff" : "2px solid rgba(255,255,255,0.2)",
                                background: c,
                                cursor: "pointer",
                                boxShadow: settingsState.btnColors[key] === c ? `0 0 10px ${c}` : "none",
                                flexShrink: 0
                              }
                            },
                            c
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  onMouseEnter: (e) => {
                    e.target.style.borderColor = "#c8a84b";
                    e.target.style.color = "#e8c870";
                  },
                  onMouseLeave: (e) => {
                    e.target.style.borderColor = "#5a4a20";
                    e.target.style.color = "#c8a84b";
                  },
                  children: "← BACK"
                }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "button",
    {
      ...
      children: [
        ...
        !isVoted && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
              padding: "5px 14px",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", style: btnStyle, onClick: onDec, children: "−" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", style: btnStyle, onClick: onInc, children: "+" })
      ]
    }
  );
}
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(SabotageGame, {});
}
